...
public class GameControl : MonoBehaviour
{
    ...
    public int bombCount = 0;                   //How many bombs have you got left?
    ...
    public void StartGame()             //Called when the player taps on the 'PLAY' button in the main menu
    {
        gameStarted = true;
    ...
    public void BirdDied()
    {
        ...
        gameOver = true;
        // Don't play the main theme anymore, instead...
    ...
    {
        //Increase the number of the power-ups
        bombCount++;
        //Play the item pick-up sound effect
        AudioManager.instance.PlayMusic("ItemPickup");
    }

    private void UpdateBombTextAndIcon()
    {
        //Show the number of power-ups
        bombText.text = "x " + bombCount.ToString();
        //The bomb button, by tapping on which the player can use the power-up, is activated only if there is a bombto use.
        if (bombCount > 0)
    ...
    public void QuitGame()                  //Called when the player taps on the 'QUIT' button in the start menu
    {
        Debug.Log("Quitting!");
        Application.Quit();
    }
